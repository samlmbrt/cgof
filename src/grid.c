...
Grid *grid_create(int width, int height) {
  ...
  /* cells/next point past the top border row */
...
void grid_randomize(Grid *grid, float density) {
  ...
  size_t buf_size = (size_t)wpr * (size_t)(grid->height + 2);
  memset(grid->buffer_a, 0, buf_size * sizeof(uint64_t));

  for (int y = 0; y < grid->height; y++) {
    for (int x = 0; x < grid->width; x++) {
      if (SDL_randf() < density) {
        grid->cells[y * wpr + x / 64] |= (uint64_t)1 << (x % 64);
      }
    }
  }
}
...
void grid_step(Grid *grid) {
  ...
  for (int y = 0; y < grid->height; y++) {
    ...
    for (int w = 0; w < wpr; w++) {
      ...
      /* 8 neighbor bitmaps via bit shifts across word boundaries */
      ...
      /*
       * Sum 8 neighbor bits into a 4-bit count per cell (bit3..bit0)
       * using a full-adder/half-adder chain. Each full adder takes
       * 3 bit-vectors and produces (carry, sum).
       */
      uint64_t t1 = n0 ^ n1;
      uint64_t s1 = t1 ^ n2;
      ...
      /* B3/S23: alive if count == 3, or count == 2 and already alive */
